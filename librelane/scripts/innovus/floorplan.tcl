...
# script would ever be invoked, because nobody with access to Innovus has
# written the commands it should contain.
#
# What this script would need to do, once written:
#   - Read the netlist and constraints from CURRENT_NL and CURRENT_SDC, the
#     environment variables TclStep.prepare_env() sets for this step's
#     declared inputs (see librelane/steps/tclstep.py:prepare_env), plus the
#     fixed variables it always sets: STEP_ID, SCRIPTS_DIR, STEP_DIR,
#     TECH_LEF, MACRO_LEFS, and one entry per accessible `self.config` key.
#   - Initialize the floorplan (die/core area, site rows, pin assignments)
#     and write DEF, netlist and SDC views to the paths in SAVE_DEF,
#     SAVE_NL and SAVE_SDC, which prepare_env() sets because
