...
class Instance:
    ...
    location : tuple[Decimal, Decimal] | None
        The physical co-ordinates of the object's origin. Leave
...
class Macro:
    ...
    ----------
    gds : list[Path]
        A list of GDSII files representing the design. At least one is required.
    lef : list[Path]
        A list of LEF files representing the design. At least one is required.
    instances : dict[str, Instance]
        A dictionary of :class:`Instance` objects representing the
    ...
    spef : dict[str, list[Path]]
        A dictionary of parasitics annotations for the various netlists of the
        Macro.
    ...
    lib : dict[str, list[Path]]
        ...
        during STA.
    spice : list[Path]
        A list of SPICE netlists constituting the design. May be
    ...
    sdf : dict[str, list[Path]]
        ...
        The keys are wildcards for timing corners supported by a certain SPEF file.
    json_h : Path | None
        A JSON file as generated by Yosys. Helpful in some flows.
    """
    ...
    json_h: Path | None = None

    def view_by_df(
        self, df: DesignFormat
    ) -> None | Path | list[Path] | dict[str, list[Path]]:
        try:
...
class Variable:
    ...
    ----------
    name : str
        A string name for the Variable. Because of backwards
    ...
    description : str
        A human-readable description of the variable. Used to
    ...
    units : str | None
        Used only in documentation: the unit corresponding to this
    ...
    pdk : bool
        Whether this variable is expected to be given a default value
        by a PDK or not.
    ...
    validator : Callable[["Variable", Any, list[str]], Any]
        A customizable validator that is run AFTER type checks
    ...
    description: str
    default: Any = None
    deprecated_names: list[str | tuple[str, Callable]] = field(default_factory=list)
    ...
    def optional(self) -> bool:
        ...
        -------
        bool
            Whether a variable's type is an `Option type <https://en.wikipedia.org/wiki/Option_type>`_.
        """
    ...
    def type_repr_md(self, for_document: bool = False) -> str:  # pragma: no cover
        ...
        ----------
        for_document : bool
            Adds HTML line breaks between sum type separators
            for easier wrapping by web browsers/PDF renderers/what have you

        Returns
        -------
        str
            A pretty Markdown string representation of the Variable's type.
        """
    ...
    def _render_table_md(
        ...
        *,
        myst_anchor_owner_id: str | None = None,
    ) -> str:  # pragma; no cover
        """
        ...
        ----------
        vars : Iterable["Variable"]
            Any iterable object returning configuration variables
        myst_anchor_owner_id : str | None
