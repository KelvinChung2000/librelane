...
:func:`librelane.flows.spec.load_flow_spec` validates against the pydantic
models, and :mod:`librelane.flows.spec_validation` and
:mod:`librelane.flows.selection_validation` then check everything a schema
...
written by hand, so a field added to :class:`~librelane.flows.spec.JobSpec`
...
:meth:`~librelane.flows.spec.FlowSpec._check_schedule_combinations` refuses.
...
# schema too.
from librelane.flows.spec import (
    _PRE_PASS_VALUE_KEY,
...
#: Mirrors ``librelane.flows.predicates._IDENTIFIER``: a bare configuration
...
def _conjunction(term: str) -> str:
    ...
    :func:`librelane.flows.predicates.parse_predicate` accepts.
    """
...
def _annotate_flow(schema: dict[str, Any]) -> None:
    ...
    _node(schema, "description")["description"] = (
        "Prose describing the flow, shown wherever flows are listed."
    )
    ...
    _node(schema, "jobs")["description"] = (
        ...
        "'steps' runs the registered job template whose id is its key."
    )
...
def _job_key_combinations() -> list[dict[str, Any]]:
    ...
    :meth:`~librelane.flows.spec.FlowSpec._check_schedule_combinations` and
