...
:mod:`librelane.engine.spec_validation` reads what a document *declares*, and it
...
:func:`join_conflicts` replays :mod:`librelane.engine.join`'s rule symbolically,
which is the only way to see a key two concurrent jobs both write. It was a
guard inside ``test/engine/test_documents.py`` first, over the shipped documents
...
* Everything here reasons over the jobs that **will run**. A job whose ``if``
  is false fires as a pass-through (:meth:`librelane.engine.engine.Workflow.run`)
  and writes nothing, so it cannot collide with anything. The
  ``test/engine/test_documents.py`` form models every job as enabled, and that is
...
constructs the :class:`librelane.engine.engine.Workflow`. So the view set a state
...
:class:`librelane.engine.engine.Workflow` is constructed. ``--skip`` makes a job
...
:func:`librelane.engine.join.join_sink_states` catches at run time with a message
...
from librelane.engine.job import ResolvedJob, resolve_jobs
from librelane.engine.predicates import MetricTerm, config_terms, metric_terms
from librelane.engine.spec import FlowSpec
from librelane.engine.spec_graph import ancestors, topological_order
...
#: and :mod:`librelane.engine.join` is what discovers the disagreement, on the
...
def validate_selection(
    ...
    jobs : Mapping[str, ResolvedJob]
        Its jobs as :func:`librelane.engine.job.resolve_jobs` resolved them,
    ...
    enabled : Set[str]
        ...
        ``if`` holds -- see :meth:`librelane.engine.engine.Workflow._enabled_jobs`.
    ...
    rings : Mapping[str, tuple[str, ...]] | None
        ...
        :meth:`librelane.engine.spec.FlowSpec.rings` returns. ``None`` or empty
        ...
        already walk :meth:`~librelane.engine.spec.FlowSpec.collapsed_edges`,
...
def _fold_rings(
    ...
    ----------
    spec : FlowSpec
        The document, for :meth:`~librelane.engine.spec.FlowSpec.collapsed_edges`,
    ...
    dict[str, ResolvedJob]
        ...
        * ``source``: the ring-entry member's (``rings[gate][0]``) -- the
          member :meth:`librelane.engine.engine.Workflow._run_loop` joins the
...
def unrepeated_runtime_terms(
    ...
    the exact term in its own ``if`` -- :class:`~librelane.engine.predicates.MetricTerm`
...
def join_conflicts(
    ...
    """
    Replays :mod:`librelane.engine.join`'s rule symbolically over a resolution.
    ...
    :func:`librelane.engine.spec_graph.topological_order` carrying exactly that --
    ...
    Walks :meth:`~librelane.engine.spec.FlowSpec.collapsed_edges` rather than
    :meth:`~librelane.engine.spec.FlowSpec.edges`, so a ring's back edge -- a
    real cycle in the declared graph -- cannot reach
    :func:`~librelane.engine.spec_graph.topological_order`, which refuses one.
...
def lost_views(
    ...
    Walks :meth:`~librelane.engine.spec.FlowSpec.collapsed_edges`, for the
...
def _alternatives_that_work(
    ...
    rings : Mapping[str, tuple[str, ...]]
        ...
        :meth:`~librelane.engine.spec.FlowSpec.collapsed_edges`'s node space,
...
def _conflict_message(
    ...
    return (
        ...
        f"librelane.engine.join has no rule for choosing. The run would stop "
