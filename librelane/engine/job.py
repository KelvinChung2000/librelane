...
from librelane.engine.predicates import MetricTerm, Term, metric_terms, parse_predicate
from librelane.engine.spec import FlowSpec, JobSpec
...
class ResolvedJob:
    ...
    empty when the job declares no ``if``. A :class:`~librelane.engine.predicates.ConfigTerm`
    is read off the flow's configuration at construction; a
    :class:`~librelane.engine.predicates.MetricTerm` is a runtime term,
