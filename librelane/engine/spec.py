...
# Re-exported: librelane.engine.spec is FlowSpecError's documented import site,
# and it lives one level down so that spec_include, which raises it before a
# FlowSpec exists, can import it without a cycle. See librelane.common.errors.
...
from librelane.engine.spec_include import INCLUDE_KEY, resolve_includes
...
class FlowSpec(BaseModel):
    ...
    @model_validator(mode="before")
    @classmethod
    def _reject_unresolved_include(cls, data: Any) -> Any:
        # An 'include' is resolved by load_flow_spec, which merges the files it
        # names into the mapping before a model is built, so one that reaches
        # here is one nothing has read. Caught rather than left to
        # extra="forbid", whose message -- "Extra inputs are not permitted" --
        # reads as "documents cannot include" rather than "this document was
        # not loaded".
        if not isinstance(data, dict) or INCLUDE_KEY not in data:
            return data
        raise FlowSpecError(
            f"A document declaring '{INCLUDE_KEY}' was handed to FlowSpec "
            f"directly. An '{INCLUDE_KEY}' names files to merge underneath "
            f"this document, which load_flow_spec does before building a "
            f"model; nothing has read them here. Load it with "
            f"load_flow_spec()."
        )
    ...
    def edges(self) -> dict[str, list[str]]:
        ...
        :mod:`librelane.engine.spec_graph` expects.
        """
    ...
    def collapsed_edges(self) -> dict[str, list[str]]:
        ...
        Returns
        -------
        :func:`librelane.engine.spec_graph.collapse` over :meth:`edges`, with
...
def load_flow_spec(source: Mapping[str, Any] | str | os.PathLike) -> FlowSpec:
    ...
    Every document the source ``include``\\ s is read and merged underneath it
    first, so the model validates one document however many files wrote it. See
    :mod:`librelane.engine.spec_include` for what merging one file into another
    means.
    ...
    """
    # Outside the wrapping below: an include is resolved before there is a
    # merged document to attribute anything to, and its errors name the file
    # the offending key is written in already, which is not necessarily this
    # one.
    resolved = resolve_includes(read_source(source))
    try:
        return FlowSpec.model_validate(resolved.mapping)
    except (FlowSpecError, ValidationError) as e:
        # Both say the same thing here: the two differ in who wrote the message,
        # never in which document it is about.
        raise FlowSpecError(f"In workflow document {resolved.origin}: {e}") from None
