...
class Flow(ABC):
    ...
    Steps : list[type[Step]]
        ...
        :class:`librelane.engine.engine.Workflow` assigns it from its resolved
    ...
    class _StepIssueSink:
        ...
        def __init__(self) -> None:
            ...
            #: :class:`librelane.engine.engine.Workflow` several jobs run at
    ...
    #: obliged to make -- :class:`librelane.engine.engine.Workflow` assigns it
    ...
    def __init__(
        ...
        *,
        ...
        # Published so that a step, a toolbox call or a notebook cell can ask
        # for the configuration without being handed it. A scope opened during
        # the run outranks this (see :meth:`start`), so a second flow built in
        # the same process replaces the singleton but cannot reach into a run
        # already under way.
        ...
        **kwargs,
        ...
        try:
            ...
            with ExitStack() as sink_stack:
                ...
                # Scoped for the run, on top of the singleton this flow already
                # published, so that a flow started while another is being
                # constructed reads its own configuration and not the one that
                # construction most recently wrote. A job that resolves a
                # configuration of its own pushes a further scope inside this
                # one.
                sink_stack.enter_context(use_config(self.config_scope))
    ...
    def dir_for_step(self, step: Step) -> pathlib.Path:
        ...
        :class:`librelane.engine.engine.Workflow` does not use this: a run
        directory keyed on a running counter cannot be resumed once steps run
        concurrently, so it names its own with
        :meth:`librelane.engine.engine.Workflow.dir_for_job_step`.
    ...
    def start_step_async(
        ...
        **kwargs,
        ...
        :class:`librelane.engine.engine.Workflow` fills with whole jobs. Call it
