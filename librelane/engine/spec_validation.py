...
Kept apart from :mod:`librelane.engine.spec` so the document models stay
...
jobs an ``if`` will enable. :mod:`librelane.engine.selection_validation` is the
...
from librelane.engine.spec import FlowSpec, FlowSpecError, JobSpec
from librelane.engine.spec_graph import ancestors, descendants
...
def validate_against_registry(spec: FlowSpec) -> None:
    ...
    # Every table the checks below share is built once, here, and handed to
    # each of them. Each is a whole-document walk -- '_reach' visits every
    # step of every job to read its 'config_vars', some 800 of them across
    # Classic's 48 jobs -- and rebuilding one inside each check made the cost
    # a function of how many checks happen to want it.
    edges = spec.edges()
    reach = _reach(spec)
    ...
    _check_sources_can_deliver(spec, keys, edges)
    _check_requirements_are_reachable(spec, views, edges)
    _check_fan_in_is_unambiguous(spec, keys, edges)
    _check_sink_join_is_unambiguous(spec, keys, edges)
    _check_job_values_are_covering(spec, reach)
    _check_schedule_values_are_covering(spec, reach)
...
def _writes_framework_metrics(job_id: str, job: JobSpec) -> bool:
    ...
    :data:`~librelane.engine.selection_validation.FRAMEWORK_METRICS` that no
    ...
    :func:`~librelane.engine.selection_validation.validate_selection`'s job.
    """
    uses = _resolved_uses(job_id, job)
    if uses is None:
        assert job.steps is not None
...
def _ambiguous_keys(
    ...
    """
    Every key both branches deliver *on their own*, sorted: what makes a join
    ambiguous, asked identically of a job's fan-in and of the flow's sink join.

    Subtracting the shared ancestry first is the whole of it. A key written
    upstream of *both* arrives down both branches, but it is one value from one
    producer and there is nothing to disambiguate; only a key each branch
    produces where the other cannot see it leaves the join with two candidates
    and no rule to choose.
    """
    shared = branches[first] & branches[second]
    exclusive_to_first = _union(keys, branches[first] - shared)
    exclusive_to_second = _union(keys, branches[second] - shared)
    return sorted(exclusive_to_first & exclusive_to_second)
...
def _check_source_keys_resolve(spec: FlowSpec) -> None:
    ...
    :data:`~librelane.engine.selection_validation.FRAMEWORK_METRICS` count as
    ...
    :func:`~librelane.engine.selection_validation.validate_selection` refuses
...
def _check_requirements_are_reachable(
    ...
    for name, job in spec.jobs.items():
        ...
        for view in sorted(_consumed_views(name, job) - available):
            ...
            # state. librelane.engine.selection_validation.lost_views keeps that
    ...
    for name, job in spec.jobs.items():
        ...
        for first, second in _pairs(job.needs):
            if second in branches[first] or first in branches[second]:
                continue
            for key in _ambiguous_keys(keys, branches, first, second):
...
def _check_job_values_are_covering(spec: FlowSpec, reach: dict[str, set[str]]) -> None:
...
def _check_schedule_values_are_covering(
    spec: FlowSpec, reach: dict[str, set[str]]
) -> None:
