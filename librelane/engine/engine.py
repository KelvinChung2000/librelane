...
from librelane.engine.flow import Flow, FlowError, FlowException
from librelane.engine.job import ResolvedJob, ToolSelection, resolve_jobs
from librelane.engine import predicates
from librelane.engine.join import join_sink_states, join_states
from librelane.engine.net import Net
from librelane.engine.resources import ResourcePools
from librelane.engine.resume import resume_key, reusable_state, write_entry
from librelane.engine.spec import FlowSpec
from librelane.engine.spec_graph import ancestors, descendants, topological_order
...
def _document_help_md(
    ...
    for job_id, job in jobs.items():
        ...
        for step in job.steps:
            ...
            if myst_anchors:
                # Addressed as a project-wide target and not as
                # './step_config_vars.md#…'. What the step page writes is an
                # explicit MyST target, ``(step-<id>)=``, which is a *label*;
                # a path-and-fragment link resolves against a document's local
                # ids instead, finds no id of that name, and warns once per
                # link -- 183 of them, which is why the docs Makefile used to
                # filter them out by name.
                result += (
                    f"  * [`{step.id}`](#step-{slugify(implementation, lower=True)})\n"
                )
...
class _Parked:
    """
    One unit of work the scheduler has decided to run: its input tokens
    already consumed and joined, its stage already started, its call bound
    and ready to hand to a worker.

    One shape for all three kinds of work :meth:`Workflow.run` submits -- an
    ordinary job, one pass of a sweep, and a ring's whole loop instance --
    which differ only in what :attr:`call` is bound to and in whether
    :attr:`resources` and :attr:`sweep_key` are set. That is what lets
    :class:`_Admissions` submit and retry all three through one code path,
    and so state the deadlock invariant :meth:`Workflow.run` documents once
    rather than at each of three submission sites and again at each of their
    three retries, where it could drift in one and not the others.

    "Parked" is the state this record is *named* for but not the only one it
    is in: it describes the work whether it went straight to a worker or had
    to wait for one, because a retry has to resubmit the identical call and
    the only way to guarantee that is to have built it once.

    A parked entry is retried on every later scheduler wake by
    :meth:`_Admissions.admit_parked`, never by blocking the scheduling
    thread: it holds its tokens but neither an executor slot nor a pool
    grant, so it can neither stall the run nor be the deadlocking half of a
    cycle -- see :meth:`Workflow.run`'s own docstring for the invariant this
    restores (task 6 review, finding C1).

    Parameters
    ----------
    submitted
        This submission's record, created once whether or not parking turns
        out to be needed, so a job's directory-of-record and step list are
        the same either way. A sweep's ``N`` passes each get their own; a
        ring's one loop instance gets one, shared across every member of
        every pass.
    call
        The bound call to hand ``get_tpe().submit``.
    resources
        The pool names this work takes one slot from each of, or ``()``.
        Empty for a ring, which never contends for a pool on the scheduling
        thread: its own ``resources`` use is per member per pass, acquired
        with a *blocking* call from inside :meth:`Workflow._run_loop` once it
        is already running on a worker. :class:`ResourcePools` returns
        immediately for an empty sequence, so this needs no branch of its
        own anywhere below.
    sweep_key
        ``(job id, pass index)`` when this is one pass of a sweep, and
        ``None`` otherwise -- how :meth:`Workflow.run` tells a resolved sweep
        pass's future from an ordinary job's.
    """

    submitted: _InFlight
    call: functools.partial[State]
    resources: tuple[str, ...] = ()
    sweep_key: tuple[str, int] | None = None


class _Admissions:
    """
    :meth:`Workflow.run`'s admission control: what is running on a worker,
    what is parked waiting for one, and what each of them holds.

    One object rather than five parallel locals threaded through every
    submission site, because the free-worker check, the pool grant and the
    three bookkeeping tables all have to move together. The deadlock
    invariant :meth:`Workflow.run` documents is precisely a statement about
    the order of the first two, and :meth:`_try_admit` below is the single
    place either is written.

    Touched only from the scheduling thread -- ``run``'s own -- so nothing
    here is locked and nothing needs to be. Workers see none of it.

    Parameters
    ----------
    pools
        The flow's resource pools, for the grant :meth:`_try_admit` takes and
        :meth:`release` returns.
    """

    def __init__(self, pools: ResourcePools) -> None:
        self.pools = pools
        ...
        # finding C1): see `Workflow.run`'s own docstring for the invariant.
        ...
        # has to account for.
        self.max_workers = get_tpe()._max_workers
        #: Every submitted future mapped to its record.
        self.pending: dict[Future[State], _InFlight] = {}
        #: Every pending future that is one pass of a sweep, mapped to which
        #: sweep job and which pass. Sweep passes live in :attr:`pending`
        ...
        #: once ``wait()`` has returned it.
        self.sweep_futures: dict[Future[State], tuple[str, int]] = {}
        #: Work whose tokens are consumed but whose pools were full, or whose
        #: executor slot was, the moment it was checked. Retried by
        #: :meth:`admit_parked` on every scheduler wake, ahead of
        #: ``net.enabled()``.
        self.parked: list[_Parked] = []
        #: Every pending future mapped to the pool names its grant holds, so
        #: :meth:`release` can return them without the caller having to
        #: remember what was taken. A ring's entry is ``()``.
        self._resources: dict[Future[State], tuple[str, ...]] = {}

    def _try_admit(self, entry: _Parked) -> bool:
        """
        Hands ``entry`` to a worker if one is free and its pools have room,
        and reports whether it did.

        The free-worker check comes first and short-circuits
        :meth:`ResourcePools.try_acquire` on failure (task 6 review, finding
        C1). Granting a pool seat to work this engine cannot also place on a
        worker is exactly the state that lets a blocked ring wait on a grant
        nothing will ever run to release. Both halves live here, in the one
        method every submission and every retry goes through, so the order
        cannot hold at one site and not another.
        """
        if len(self.pending) >= self.max_workers:
            return False
        if not self.pools.try_acquire(entry.resources):
            return False
        future = get_tpe().submit(entry.call)
        self.pending[future] = entry.submitted
        self._resources[future] = entry.resources
        if entry.sweep_key is not None:
            self.sweep_futures[future] = entry.sweep_key
        return True

    def submit(self, entry: _Parked) -> None:
        """Hands ``entry`` to a worker, or parks it for a later wake."""
        if not self._try_admit(entry):
            self.parked.append(entry)

    def admit_parked(self) -> None:
        """
        Retries everything parked, submitting whatever now fits and leaving
        the rest parked in its original relative order.

        Called once at the top of :meth:`Workflow.run`'s scheduling loop, on
        every wake: after ``wait()`` returns (something completed and may
        have freed a slot or a pool grant) and after a pass-through sweep
        loops back around (which frees neither, but costs nothing extra to
        re-check). Non-blocking throughout -- every admission is
        ...
        The free-worker check inside :meth:`_try_admit` re-reads the live
        :attr:`pending` on every entry rather than being cached once for the
        whole call: admitting one parked entry grows it by one, and a second
        entry considered in the same call must see that growth before
        deciding whether *it* still fits.
        """
    ...
    def resolved(
        self, future: Future[State]
    ) -> tuple[_InFlight, tuple[str, int] | None]:
        """
        Drops a resolved future from the pending tables.

        Returns
        -------
        Its record, and its ``(job id, pass index)`` if it was one pass of a
        sweep or ``None`` if it was an ordinary job or a ring.

        The pool grant is deliberately *not* returned here. A sweep pass
        releases its own the moment it resolves but is not settled until its
        siblings have, so the two happen at different points; see
        :meth:`release`.
        """
        return self.pending.pop(future), self.sweep_futures.pop(future, None)

    def release(self, future: Future[State]) -> None:
        """
        Returns whatever pool grant ``future`` held.

        Called from the ``finally`` of whichever branch settles the future,
        so a slot is returned on every exit path, success or failure. A ring
        took none (its pool use is internal to
        :meth:`Workflow._run_loop`), and neither did a job naming no pool;
        both recorded ``()`` and this is a no-op for them.
        """
        self.pools.release(self._resources.pop(future, ()))


class Workflow(Flow):
    """
    Runs a :class:`librelane.engine.spec.FlowSpec`.
    ...
    config
        As :meth:`librelane.engine.Flow.__init__`.
    config_override_strings
        As :meth:`librelane.engine.Flow.__init__`. Read twice, once here for
    ...
    initial_views
        ...
        :func:`librelane.engine.selection_validation.supplied_views` reads them
        ...
        :meth:`librelane.engine.Flow.start`, which is given it as
    ...
    class Config(Flow.Config):
        TOOLS: dict[str, str | list[str]] | None = variable(
            None,
            description=(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # librelane.engine.selection_validation's docstring is where the reasons
        ...
        self.iteration_scopes: dict[tuple[str, int], ConfigScope] = {
            key: build_scope(resolved, model_type=self.config_model_type)
            for key, resolved in self.iteration_configs.items()
        }
    ...
    def _resolve_resource_pools(self) -> ResourcePools:
        ...
        ResourcePools
            ...
            given: :meth:`~librelane.engine.spec.FlowSpec._check_resource_pool_literal_capacities`
            ...
            :func:`librelane.engine.spec_validation._check_resource_variable_capacities_are_declared_ints`
    ...
    def _resolve_job_configs(
        ...
        that ``librelane.engine.spec_validation`` enforces is not a uniqueness
        ...
        for job_id, job in self.spec.jobs.items():
            if not job.values:
                continue
            self._require_layerable(config, f"Job '{job_id}'", "a 'with' block")
            # Two layers rather than one merged mapping: the job overrides the
            # document, anything neither sets still comes from the document,
            # and each key is attributed to whichever of the two wrote it.
            job_configs[job_id] = self._load_layer(
                config,
                config_override_strings,
                load_kwargs,
    ...
    def _require_layerable(
        self, config: AnyConfigs, subject: str, declares: str
    ) -> None:
        """
        Parameters
        ----------
        config : AnyConfigs
            The configuration this flow was constructed from.
        subject : str
            What declared the layer, named as the message should open: ``"Job
            'x'"``, ``"The ring gated by 'x'"``, ``"Sweep job 'x'"``.
        declares : str
            What it declared, in the message's own words: ``"a 'with' block"``
            or ``"'iterations'"``.

        Raises
        ------
        FlowException
            If ``config`` is an already-resolved :class:`librelane.config.Config`.
            A ``with`` block and an ``iterations`` entry are both resolved by
            layering values into the loader's *sources*, and a resolved
            configuration no longer has any. Running the job against the
            flow's configuration instead would discard values the document
            asked for without saying so, which is why this refuses rather
            than falling back.
        """
        if not isinstance(config, Config):
            return
        raise FlowException(
            f"{subject} of flow '{self.spec.name}' declares {declares}, but "
            f"this flow was constructed from a configuration that is already "
            f"resolved, whose sources are no longer available to layer it "
            f"into. Pass the design's configuration file or mapping instead."
        )
    ...
    def _load_layer(
        ...
        *,
        ...
        """
        Resolves one configuration, with an optional per-job and per-pass
        layer on top of the flow's own.

        The three resolution sites -- a job's ``with`` block, a ring member's
        pass, a sweep job's pass -- differ only in those two layers. Eight
        further arguments are the same at all three, and the process
        selection among them has to be read out of ``load_kwargs`` rather
        than off ``self``, so writing them out per site is where a ``--pdk``
        that reached two of the three and not the third would come from.

        Parameters
        ----------
        config : AnyConfigs
            The design configuration, exactly as it was handed to
            ``Flow.__init__``. The layers go into the loader's *sources*, not
            onto a resolved result: a job's ``with`` beats the document's and
            the PDK's, and loses to the design's and to ``--config-override``,
            and only the loader knows which of those supplied a given value.
        config_override_strings : Sequence[str] | None
            As :meth:`librelane.engine.Flow.__init__`.
        load_kwargs : Mapping[str, Any]
            The remaining keyword arguments ``Flow.__init__`` received; the
        ...
        job_values : tuple[str, Mapping[str, Any]] | None
            A job id and its own ``with`` values, or ``None`` for a job that
            sets none.
    ...
    def _resolve_iteration_configs(
        ...
        """
        # Who declares the schedule and who reads it apart only for a ring,
        # where the gate declares it and every member reads it; a sweep job
        # is both. Collected first, and the schedule with them so that a
        # ring's is expanded once rather than once per member, because that
        # is the whole of the difference between the two shapes -- the
        # resolution loop below is then written once.
        scheduled: list[tuple[str, str, tuple[dict[str, Any], ...]]] = []
        for gate, members in self.rings.items():
            gate_schedule = self.spec.jobs[gate].schedule()
            if not gate_schedule:
                continue
            self._require_layerable(
                config, f"The ring gated by '{gate}'", "'iterations'"
            )
            scheduled.extend((gate, member, gate_schedule) for member in members)
        for job_id, job_spec in self.spec.jobs.items():
            job_schedule = job_spec.schedule()
            if job_spec.select is None or not job_schedule:
                continue
            self._require_layerable(config, f"Sweep job '{job_id}'", "'iterations'")
            scheduled.append((job_id, job_id, job_schedule))

        configs: dict[tuple[str, int], _ResolvedConfig] = {}
        for owner, reader, schedule in scheduled:
            reader_spec = self.spec.jobs[reader]
            job_values = (reader, reader_spec.values) if reader_spec.values else None
            for k, entry in enumerate(schedule, start=1):
                configs[(reader, k)] = self._load_layer(
                    config,
                    config_override_strings,
                    load_kwargs,
                    job_values=job_values,
                    iteration_values=(owner, k, entry),
                )
        return configs
    ...
    def run(
        ...
        **kwargs,
        ...
        worker count to have room for -- the free-worker check that
        :meth:`_Admissions._try_admit` makes immediately before every
        ``try_acquire``, short-circuiting it on failure. Every submission
        this method makes goes through that one method (an ordinary job, a
        sweep pass, a ring, and each one's retry), so the order holds at all
        of them or at none. Work refused a pool grant or refused a worker
        parks (:class:`_Parked`) instead of blocking. Together this means:
        anything blocked in :meth:`ResourcePools.acquire` is always waiting
        on a grant held by genuinely running work, which is therefore making
        progress and will eventually release it, which is what makes the
        wait finite.
        ...
        #: What is on a worker, what is parked waiting for one, and what each
        #: holds. Every submission below goes through it.
        admissions = _Admissions(self.pools)
        ...
        while True:
            ...
            if not failures:
                for name in net.enabled():
                    ...
                    started = False
                    try:
                        # 'name' is a node of the *collapsed* net, so it is
                        # either an ordinary job's id or a ring's gate id.
                        # Both are read the same way from here on: consume
                        # this node's tokens, join them into the one state
                        # that decides the node's 'if', and either fire it
                        # through or hand it to a worker. Only the two ends
                        # differ, so only the two ends branch.
                        ring_members = self.rings.get(name)
                        job = self.jobs[name]
                        if ring_members is None:
                            state_in = join_states(
                                self._tokens_for(net, job), job.source, name
                            )
                            subject = f"job '{name}'"
                        else:
                            ...
                            # fires as a single pass-through. '_run_loop'
                            # needs them still attributed per member, so the
                            # flattening below is for this check only.
                            member_tokens = self._tokens_for_ring(
                                net, name, ring_members
                            )
                            flattened: dict[str, State] = {}
                            for member_external in member_tokens.values():
                                flattened.update(member_external)
                            state_in = join_states(flattened, job.source, name)
                            subject = f"loop gated by '{name}'"
                        self.progress_bar.start_stage(name)
                        started = True
                        # --skip can never name a ring member, checked in
                        # _plan, so for a gate this is always False; it is
                        # asked uniformly rather than branched on, because a
                        # second spelling of the pass-through rule is a
                        # second place for it to be wrong.
                        reason = self._pass_through_reason(
                            job, name in plan.skipped, state_in
                        )
                        if reason is not None:
                            logger.info(f"Skipping {subject}: {reason}.")
                            net.fire(name, state_in)
                            outputs[name] = state_in
                            self.progress_bar.end_stage()
                            fired_pass_through = True
                            continue
                        # Admission is the last decision on this thread, after
                        # every pass-through check: a pass-through acquires
                        # nothing because it runs nothing, and this point is
                        # reached only for work that is genuinely about to run.
                        if ring_members is not None:
                            submitted = _InFlight(name)
                            # No 'resources': a ring never contends a pool
                            # from this thread (its members' own are acquired
                            # blocking, from inside _run_loop, once it has a
                            # worker). It still needs a free worker to be
                            ...
                            # finding C1) -- and that half of the check is
                            # exactly what _Admissions applies to it.
                            admissions.submit(
                                _Parked(
                                    submitted,
                                    functools.partial(
                        ...
                        elif job.select is not None:
                            # Fan-out happens here, on this thread, never
                            # inside a worker: a worker blocked waiting on its
                            # own children would deadlock a saturated pool the
                            # moment every thread is one of those workers. The
                            # tokens were already consumed and joined once,
                            # above -- every pass reads the identical
                            # 'state_in' -- and start_stage already ran once,
                            # above too, so the N submissions below are this
                            # one stage, not N of them; end_stage is deferred
                            # to settlement.
                            n = len(job.iterations)
                            sweeps[name] = _SweepProgress(name, n)
                            for k in range(1, n + 1):
                                pass_submitted = _InFlight(f"{name} (pass {k})")
                                # Each pass is admitted on its own, not once
                                # for the whole sweep, so a two-seat pool runs
                                # a five-point sweep two passes at a time
                                ...
                                admissions.submit(
                                    _Parked(
                                        ...
                                        functools.partial(
                                            ...
                                            pass_index=k,
                                            scope=self.iteration_scopes[(name, k)],
                                        ),
            ...
            wait_targets: list[Future[Any]] = list(admissions.pending)
            if admissions.parked:
                with sentinel_lock:
                    if release_sentinel is None or release_sentinel.done():
                        release_sentinel = Future()
            ...
            for future in done:
                if future is release_sentinel:
                    ...
                    continue
                finished, sweep_key = admissions.resolved(future)
                ...
                try:
                    ...
                    assert not admissions.pending, (
                        "a reproducible unwound while "
                        f"{sorted(o.name for o in admissions.pending.values())} "
                        "were still running"
                    )
    ...
    def explain(
        ...
        *,
        ...
        for collapsed_id in topological_order(edges):
            ...
            # ordinary job. Every verdict below is reached the same way for
            # each id in 'row_ids', because every fact this method reports
            # about a ring -- whether it is in the target subgraph, whether
            # its gate's 'if' stops it -- is a fact about the whole loop
            # instance, not about one member.
            row_ids = ring_members if ring_members is not None else (collapsed_id,)
            for job_id in row_ids:
                dispositions.append(
                    JobDisposition(
                        job_id,
                        tuple(self.jobs[job_id].needs),
                        *self._verdict(job_id, collapsed_id, ring_members, plan),
                    )
                )
        return Explanation(
    ...
    def _verdict(
        ...
        job_id: str,
        collapsed_id: str,
        ring_members: tuple[str, ...] | None,
        plan: _RunPlan,
    ) -> tuple[bool, str, str | None]:
        """
        Classifies one row of :meth:`explain`'s job table.

        Parameters
        ----------
        job_id : str
            The job the row is about.
        collapsed_id : str
            The node of the collapsed graph it belongs to: its own id, or its
            ring's gate id. Every question about the *graph* -- selection
            above all -- is asked of this and not of ``job_id``, because a
            bare member is not a node the graph has.
        ring_members : tuple[str, ...] | None
            ``collapsed_id``'s members if it is a ring, and ``None`` if it is
            an ordinary job.
        plan : _RunPlan
            What the invocation selects, from the same derivation
            :meth:`run` executes.

        Returns
        -------
        ``(will_run, reason, mechanism)``, the three fields of
        :class:`JobDisposition` that vary by row -- returned rather than
        appended, so the row itself is constructed once by the caller and no
        branch here can spell ``job_id`` or ``needs`` differently from
        another.

        The order of the tests is the order the mechanisms actually compose,
        and each one returns, so no later test can contradict an earlier one.
        """
        ...
        if collapsed_id not in plan.selected:
            ...
            if plan.reproducible_at is not None:
                return (
                    False,
                    f"--reproducible runs only '{plan.reproducible_at[0]}' "
                    f"and its ancestors",
                    "not-in-reproducible",
                )
            return False, "not in the --target subgraph", "not-in-target"

        if ring_members is not None:
            gate_job = self.jobs[collapsed_id]
            # A ring member can never itself be named by --skip (refused in
            # _plan) or by --reproducible (refused in _resolve_reproducible),
            # so only the gate's own configuration term can stop the whole
            # loop instance before it starts, exactly as it does in run().
            gate_reason = self._pass_through_reason(gate_job, skipped=False)
            if gate_reason is not None:
                return False, gate_reason, "condition"
            bound = len(gate_job.iterations) or gate_job.max_passes
            if job_id == collapsed_id:
                return True, f"gate of the loop, at most {bound} passes", "loop"
            return (
                True,
                f"loop gated by '{collapsed_id}', at most {bound} passes",
                "loop",
            )

        # The same call the run makes, so the two cannot disagree about which
        # variable stopped a job or how its absence is worded. No state is
        # passed: a runtime term is not decided until the run reads the job's
        # actual input state, and this is reported separately below.
        skipped = job_id in plan.skipped
        reason = self._pass_through_reason(job, skipped)
        if reason is not None:
            return False, reason, "skip" if skipped else "condition"

        if plan.reproducible_at is not None and job_id == plan.reproducible_at[0]:
            # Reached only for a job that is neither skipped nor stopped by a
            # condition: the plan refuses a reproducible for one that is,
            # because this run would never execute the step.
            #
            # The one job that neither runs in full nor is excluded: the
            # steps ahead of the named one run, the named one is packaged
            # instead of run, and the run stops there. 'will_run' is
            # therefore true only when some step of it runs, which is
            # exactly when the named step is not its first.
            step_index = plan.reproducible_at[1]
            step_id = job.steps[step_index].id
            if step_index == 0:
                return (
                    False,
                    f"--reproducible packages its first step '{step_id}', so "
                    f"no step of this job runs",
                    "reproducible",
                )
            return (
                True,
        ...
        if runtime_terms:
            # --skip and a false configuration term are decided now, and both
            # already returned above; a runtime term is not decided until the
            # run reads the job's actual input state, so the job counts as
            # enabled here -- the same optimistic reading _enabled_jobs gives
            # it -- and this row says so rather than claiming the answer
            # either way.
            terms_text = " and ".join(
            ...
            return (
                True,
                f"runtime term(s) {terms_text} are decided at run time "
                f"against the job's input state",
                "condition (runtime)",
            )
        ...
        if job.select is not None:
            ...
            return (
                True,
                f"sweep of {len(job.iterations)} points, keeps {metric} {direction}",
                None,
            )

        return True, "will run", None
    ...
    def _variable_dispositions(self) -> tuple[VariableDisposition, ...]:
        ...
        tuple[VariableDisposition, ...]
            ...
            order :meth:`librelane.engine.Flow.get_all_config_variables` returns
    ...
    def _final_state(
        ...
        JoinConflictError
            ...
            :func:`librelane.engine.join.join_sink_states` rather than
            :func:`librelane.engine.join.join_states`, because the two remedies
        ...
        NetError
            ...
            :meth:`librelane.engine.net.Net.sink_tokens` raises and which means
    ...
    def _member_config(self, gate: ResolvedJob, member: str, k: int) -> ConfigScope:
        """
        Returns
        -------
        The scope ``member`` reads on pass ``k``: the pre-resolved
        ``(member, k)`` entry of :attr:`iteration_scopes` when the ring's
        ...
        otherwise this member's own job scope (or the flow's, if it sets
        none), exactly as an ordinary job outside any ring reads it.
        """
        if gate.iterations:
            return self.iteration_scopes[(member, k)]
        return self.job_scopes.get(member, self.config_scope)
        ...
        for k in range(1, bound + 1):
            for index, member in enumerate(members):
                ...
                if member == gate:
                    ...
                    reason = self._pass_through_reason(
                        member_job, skipped=False, state_in=input_state
                    )
                if reason is not None:
                    logger.info(
                    ...
                    continue
                scope = self._member_config(gate_job, member, k)
    ...
    def _enabled_jobs(self) -> set[str]:
        ...
        set[str]
            ...
            (:mod:`librelane.engine.selection_validation` folds a ring into
    ...
    def _pass_through_reason(
        self, job: ResolvedJob, skipped: bool, state_in: State | None = None
    ) -> str | None:
        """
        Parameters
        ----------
        ...
        skipped : bool
            Whether ``--skip`` names it.
        state_in : State | None
            The joined input state to decide the job's runtime (``metric::``)
            terms against, or ``None`` to leave them undecided.

            ``None`` is not "no state to hand it" but a different question:
            it asks only what is knowable before the job's predecessors have
            run, which is what :meth:`_enabled_jobs`, :meth:`explain` and
            :meth:`_plan` need -- the spec's optimistic rule, under which a
            job whose ``if`` holds only runtime terms counts as enabled. The
            scheduling loop passes the state and gets the whole answer.

        Returns
        -------
        Why this job fires without running, or ``None`` if it runs.
        A conjunction is false when any one of its terms is false, and the
        reason names every false configuration variable so a document author
        does not have to flip them one at a time.

        The three kinds of term are answered in a fixed order, which is why
        they are one method and not three: ``--skip`` and a false
        configuration variable are both settled at construction and win
        outright over a runtime term, which cannot be decided until the run
        reads the job's actual input state. A caller that asked them in the
        other order would report a missing metric for a job that a false
        ``if`` variable had already stopped.

        The configuration is read off the flow's and never off the job's own,
        even for a job that sets the gate variable in its ``with`` block.
        ``if`` is the engine's question about whether to fire the job at all,
        asked before the job exists to answer for itself; a job that could
        switch its own gate on would make every ``if`` naming a variable that
        job also sets unconditionally true, which is not a gate. This is a
        decision, not an oversight: a per-job ``with`` changes what the job's
        *steps* read, and nothing else.

        Raises
        ------
        FlowError
            Propagated from
            :func:`~librelane.engine.predicates.evaluate_metric_term` when
            ``state_in`` is given: the term's metric is absent from
            ``state_in.metrics``, or its observed value is not numeric.
        """
        if skipped:
    ...
    def _run_job(
        ...
        scope = self.job_scopes.get(job.id, self.config_scope)
    ...
    def _execute_steps(
        ...
        *,
        ...
        scope : ConfigScope
            The configuration this execution's steps read, opened around the
            loop below rather than handed to each step, so that anything a
            step reaches into reads it too. An ordinary job's own
            (:meth:`_run_job` resolves it before calling here); a ring
            member's is its iteration scope on a scheduled pass or its own job
            scope otherwise, resolved by :meth:`_run_loop`, because which
            configuration applies is a fact about the pass, not about the job.

            Opening it here and not once per flow is what keeps concurrent
            executions apart: this runs on its own worker thread, and
            :func:`librelane.config.use_config` is scoped to the execution
            that opens it, so a five-point sweep's five passes each read their
            own values.
        ...
        """
        current = state_in
        # Held open across every step, and across everything they call: a step
        # is constructed without a configuration and finds this one.
        with use_config(scope):
            for index, cls in enumerate(job.steps):
                step = cls(
                    state_in=current,
                    # The logging layer keys a running step on its id: the
                    # loguru sink filter that routes records into the step's
                    # own step.log, LiveLog's registry of what is live, and the
                    # progress row read off it. Two jobs running the same step
                    # class at once would share that key, so each one's records
                    # would land in both step.log files, the second
                    # registration would overwrite the first's display, and
                    # whichever finished first would unregister the other.
                    # Step's initializer documents a per-instance id as the way
                    # to disambiguate one step class used more than once in a
                    # flow; the job (and, for a pass, the pass number) is what
                    # distinguishes them.
                    id=(
                        f"{cls.id} ({job.id})"
                ...
                if (job.id, index) == reproducible_at:
                    # Before the resume check, and without the rmtree below:
                    # the step is not going to run, so neither reusing its
                    # previous result nor deleting it is meaningful. It is also
                    # not appended to 'steps', because it never ran.
                ...
                try:
                    ...
                    # A deferred error says "flag this at the end, keep going",
                    # and where it was raised decides what "keep going" carries
                    # on from. A gate raises after run() has returned, so the
                    # step has real outputs -- OpenROAD.DetailedRouting
                    # reporting DRC violations still routed the design -- and
                    # dropping them would hand every later step the views from
                    # before it ran. A step that defers from inside run() never
                    # reached that point and has none, so the flow carries on
                    # with the state it already had. start() clears state_out
                    # before running, so the attribute answers which of the two
    ...
    def _check_contract(self, job: ResolvedJob, state: State) -> None:
        ...
        :func:`librelane.engine.job.resolve_jobs` as the union of its steps'
