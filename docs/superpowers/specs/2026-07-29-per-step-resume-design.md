...
Superseded, 2026-07-30. `docs/superpowers/specs/2026-07-30-flow-mechanism-unification-design.md`
deletes both flows, after which every flow in the tree is sequential and this
non-goal has no remaining subject. It becomes true vacuously rather than false,
so nothing built here is invalidated. The one consequence is that
`Flow.dir_for_step`'s optional `position` parameter
(`librelane/flows/flow.py:816-841`) exists for a case that no longer occurs.
Whether it stays optional is a decision for whichever spec reintroduces a
non-sequential flow, which is the workflow engine.
