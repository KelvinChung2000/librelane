...
Superseded, 2026-07-30. Both named flows are deleted by
`docs/superpowers/plans/2026-07-30-flow-mechanism-unification.md`, Task 8, after
which no flow omits the position. The parameter stays optional, because the
workflow engine reintroduces flows that need to omit it; that decision belongs to
the engine's spec. Task 8 Step 3 of that plan rewrites the one docstring in
`librelane/flows/flow.py` that named `Optimizing` as the example.
...
- That resume applies to sequential flows. `Optimizing` and
  ...
  Superseded, 2026-07-30. Both flows are deleted by
  `docs/superpowers/plans/2026-07-30-flow-mechanism-unification.md`, Task 8. That
  plan's Task 8 Step 7 rewrites this passage in
  `docs/source/usage/resuming_runs.md` at the same commit as the deletion, so do
  not write the "which flows participate" section from this bullet if you are
  executing this plan after that one.
